...
const AuthReducer = (state: initialStateType = initialState, action: AnyAction): initialStateType => {
    switch (action.type) {
...
export const getMeAC = (login: string, email: string, id: number): getMeACType => {
    return {
...
export const getMeThunk = (): ThunkType => async dispatch => {
    ...
    if (response.resultCode === 0) {
        dispatch(getMeAC(response.data.login, response.data.email, response.data.id))
...
export const loginThunk = (email: string, password: string, saveMe: boolean): ThunkType => async dispatch => {
    let response = await AuthAPI.login(email, password, saveMe)
