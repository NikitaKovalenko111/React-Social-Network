...
import {
    ...
    getIsOwnerSelector,
    getLookingForAJobSelector,
    getLookingForAJobDescriptionSelector,
    getMyPhotoSelector,
    getStatusSelector,
...
import { Form, Input, Select, Space, Alert } from 'antd'
import {
    FacebookOutlined,
    GithubOutlined,
    YoutubeOutlined,
...
const Profile: React.FC<ProfilePropsType> = React.memo((): JSX.Element => {
    ...
    const lookingForAJob: boolean | null = useSelector(
        getLookingForAJobSelector
    )
    const lookingForAJobDescription: string = useSelector(
        getLookingForAJobDescriptionSelector
    )
    ...
    useEffect(() => {
        ...
        if (userId === Number(params.userId)) setIsOwner(true)
            ...
            >
                {({
                    ...
                    <Form
                        ...
                        className={cn(styles.avatarWrapper, {
                            [styles.avatarWrapperIsOwner]: isOwner,
                        })}
                    >
                    ...
                    {!ProfileDataForm ? (
                        <div className={cn(styles.notForm)}>
                            <Space direction="vertical">
                                <p className={cn(styles.lookingForAJob)}>
                                    Ищет работу:{' '}
                                    <span
                                        className={cn(
                                            lookingForAJob
                                                ? styles.lookingForAJobGreen
                                                : styles.lookingForAJobRed
                                        )}
                                    >
                                ...
                                {lookingForAJob && (
                                    <p
                                        className={cn(
                                            styles.lookingForAJobDescription
                                        )}
                                    >
                                        Мои навыки: {lookingForAJobDescription}
                                    </p>
                                )}
                                {aboutMe && (
                                    <p className={cn(styles.aboutMe)}>
                                        Обо мне: {aboutMe}
                                    </p>
                                )}
                                <div
                                ...
                                {isOwner && !ProfileDataForm && (
                                    <button
                                        onClick={() => setProfileDataForm(true)}
                        ...
                        <Formik
                            initialValues={{
                                lookingForAJob: lookingForAJob ? '1' : '0',
                                fullName: fullName,
                                lookingForAJobDescription:
                                    lookingForAJobDescription === ''
                                        ? null
                                ...
                                aboutMe: aboutMe,
                                mainLink:
                                    /*!!contacts.mainLink ? */ contacts.mainLink /*.substring(selectBeforeValue.length, contacts.mainLink.length - selectAfterValue.length) : null*/,
                            ...
                            onSubmit={async (values, { setSubmitting }) => {
                                ...
                                await changeProfile(
                                    userId,
                                    values.fullName,
                                    values.aboutMe as string,
                                    Boolean(Number(values.lookingForAJob)),
                                        ...
                                        {values.lookingForAJob === '1' && (
                                            ...
                                            >
                                                ...
                                                <Input
                                                    ...
                                                    defaultValue={
                                                        values.lookingForAJobDescription as string
                                                    }
                                            ...
                                            >
                                                ...
                                                <div>
                                                    <Input
                                                        ...
                                                        placeholder="Ссылка на ваш twitter"
                                                    />
                                                    {errors.twitter &&
                                                        touched && (
                                                            <Alert
